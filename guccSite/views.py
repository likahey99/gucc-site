...
from guccSite.forms import UserForm, UserProfileForm, CategoryForm, GearForm, AdminForm, PageContentsForm, \
    BackgroundImageForm, LogoImageForm, BookingCommentsForm, IconImageForm
from guccSite.models import UserProfile, Category, Gear, Booking, AdminPassword, BookingComments, PageContents, \
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("edit-link-id"):
            try:
                ...
                link.save()
                return redirect(reverse('guccSite:index'))
    ...
    return render(request, 'guccSite/index.html', context_dict)
...
def register(request):
    ...
    if registered:
        return render(request, 'guccSite/login.html', context=context_dict)
        ...
        return render(request, 'guccSite/register.html', context=context_dict)
...
def about(request):
    ...
    if request.method == 'POST':
        ...
        if not errors:
            ...
            context_dict['content'] = content
            context_dict['admin'] = True
            return redirect(reverse('guccSite:about'))

    return render(request, 'guccSite/about.html', context_dict)
...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        if not errors:
            ...
            context_dict['content'] = content
            context_dict['admin'] = True
            return redirect(reverse('guccSite:contact'))
...
def category_menu(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('guccSite:find-gear'))
        else:
            for error_category in form.errors:
                for error in form.errors[error_category]:
                    errors.append(error)
        context_dict['errors'] = errors
    context_dict['category'] = None
    return render(request, 'guccSite/category_menu.html', context_dict)
...
def view_gear(request, gear_name_slug):
            ...
            if request.POST.get("edit-gear"):
                ...
                if form.is_valid():
                    ...
                    if gear_name_slug != new_slug:
                        ...
                        if not existing_obj:
                            gear = form.save()
                            return redirect(reverse('guccSite:view-gear',
                ...
                if passwords[0].password == readable_hash:
                    gear.delete()
                    return redirect(reverse('guccSite:view-category',
                ...
                if request.POST.get('edit_id'):
                    if request.POST.get('comment'):
                        try:
                            ...
                            comment.save()
                            return redirect(reverse("guccSite:view-gear", kwargs={'gear_name_slug': gear_name_slug}))
                    ...
                    if request.user:
                        ...
                        return redirect(reverse("guccSite:booking", kwargs={'booking_id': borrow.id }))
    ...
    return render(request, 'guccSite/view_gear.html', context_dict)
...
def account(request):
    ...
    context_dict['errors'] = errors
    return render(request, 'guccSite/account.html', context_dict)
...
def admin_error(request):
    context_dict = {'categories': Category.objects.all()}
    context_dict['category'] = None
    return render(request, 'guccSite/admin_error.html', context=context_dict)
...
def view_category(request, category_name_slug):
    ...
    if request.method == 'POST':
        ...
        context_dict['errors'] = errors
    return render(request, 'guccSite/category.html', context=context_dict)
...
def booking(request, booking_id):
    ...
    try:
        ...
        if booking.user != user_profile and not user_profile.adminStatus:
            return redirect(reverse("guccSite:admin-error"))
            ...
            if request.POST.get('edit_id'):
                if request.POST.get('comment'):
                    try:
                        ...
                        comment.comment = request.POST.get('comment')
                        comment.save()
                        return redirect(reverse("guccSite:booking", kwargs={'booking_id': booking.id}))
                    except BookingComments.DoesNotExist:
                        errors.append("Error: Booking does not exist.")
            elif request.POST.get('delete_id'):
                if request.POST.get('delete_password'):
    ...
    return render(request, 'guccSite/booking.html', context=context_dict)
...
def user(request, user):
    ...
    return render(request, 'guccSite/user.html', context_dict)

def handler404(request, exception):
    context_dict = {'categories': Category.objects.all()}
    context_dict['category'] = None
    return render(request, 'guccSite/404.html', context=context_dict)