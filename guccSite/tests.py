...
import guccSite.forms
import populate_guccSite
from guccSite.forms import CategoryForm, GearForm, UserProfileForm, AdminForm
from guccSite.models import UserProfile, Category, Gear, Booking, AdminPassword
...
class ViewTests(TestCase):
    ...
    def testAbout(self):
        url = reverse("guccSite:about")
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, "guccSite/about.html")

    def testRedirect(self):
        url = reverse("guccSite:add-category")
        self.client.logout()
        response = self.client.get(url, follow=True)
        self.assertRedirects(response, "/gucc-site/login/?next=/gucc-site/add-category/")
